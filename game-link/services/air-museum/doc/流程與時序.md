...
**共同 API**：兩端皆須先認證；玩家首登亦走此 API（auth 與 register 已合併）。

| 路由 | packType | 方向 | 說明 |
|------|----------|------|------|
| auth/validate | request | Client → Server | 認證（含首登註冊）。token 三種格式：<br>① 投影端：`device=projector`<br>② 玩家續玩：`key=<uid>&device=player`<br>③ 玩家首登：`register&name=<url-encoded>&age=<n>&sex=<n>&device=player`（伺服器於 handler 內 `db.CreatePlayer` 建檔後直接綁連線） |
| auth/validate | response | Server → Client | ValidateResp { uid, msg? }。首登時 uid 即新建的 `player.uid` |
...
**玩家資料（DB）**：Postgres 表 **player**（uid、session、name、age、sex、avatar_glasses、avatar_helmet、avatar_eyes、avatar_mouth、game_score、landing_score、ranking、creattime），由服務啟動時建立；`creattime` 為建立時間（預設 `NOW()`）；顯示用資料由各端自行處理。
...
**Token**：三種格式共存於同一路由 `auth/validate`（見上表）；投影端 `device=projector`、玩家續玩 `key=<uid>&device=player`、玩家首登 `register&name=...&age=...&sex=...&device=player`（`name` 需 URL-encode）。**手柄輸入**：axis_x、axis_y（-1～1）、seq；約 15～20 Hz，投影機用固定速度積分。
...
**首登註冊流程**：玩家第一次進入時 token 帶 `register&name=...&age=...&sex=...&device=player`。Server 於 `HandleAuthValidate` 內呼叫 `db.CreatePlayer(name, age, sex)` 建檔取得 `player.uid`，再沿用續玩流程完成 `SetUID / Register / ValidateResp{uid}`。Client 收到 uid 後保存，下次重啟改用 `key=<uid>&device=player`。
...
| --------------- | ------------ | ------ | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Client → Server | 0 (request)  | auth   | validate | ValidateReq { token, gate_sid?, device? }。token 可為：`device=projector`、`key=<uid>&device=player`、`register&name=<url-encoded>&age=<n>&sex=<n>&device=player` |
| Server → Client | 1 (response) | auth   | validate | ValidateResp { uid, msg? }。首登情境 uid 即新建的 player.uid                                                                                                      |
...
- **建立個人檔案**：由另一 API 實作，供玩家建立／更新頭像配件（avatar_glasses 等）與 plane 等資料並寫入 Postgres。
